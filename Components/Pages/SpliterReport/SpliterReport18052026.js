...
export default function SpliterReport({
  ...
  const fetchReportData = async (filters = {}, Master, allData = false) => {
    try {
      ...
      setSpData(response);
      setDropdownFilteredRd3(null);   // ← reset dropdown filter on fresh fetch
      setFirstPanelFilterValue("");   // ← reset dropdown selection too
  ...
  // Always use dropdown-filtered rd3 as the base for panels
  ...
  const uniqueValuesForSecondPanel = useMemo(() => {
    if (!spliterReportSecondPanel || !spData?.rd2 || !activeRd3.length) return [];
    const map = spData.rd2[0];
  ...
  }, [activeRd3, spData, spliterReportSecondPanel, selectedFirstPanelKey, spliterReportFirstPanel]);
  ...
  const handleFirstPanelSelection = (value, rd3Override = null) => {
    const base = rd3Override ?? activeRd3;  // ← use override if provided
    ...
    if (value === "__ALL__") {
      ...
      const summary = calculateSummaryForFirstPanel(base);
  ...
  const handleSecondPanelSelection = (value) => {
    ...
    // ✅ Use activeRd3 instead of spData.rd3
    const firstFiltered =
        ...
        >
          {collapsed[0] ? (
            ...
            <div>
              ...
              >
                {!largeData && <div style={{ display: 'flex', gap: '10px' }}>
                  <DualDatePicker
                    ...
                    >
                      ...
                      <Select
                        ...
                        onChange={(e) => {
                          ...
                          if (!val) {
                            setDropdownFilteredRd3(null); // null means "use full spData.rd3"
                            return;
                          }
                              ...
                              style={{
                                background:
                                  selectedSecondPanelKey === v
                                    ...
                                    : "rgb(244 241 241 / 36%)",
                                color:
                                  selectedSecondPanelKey === v ? "white" : "#424651",
                                fontWeight:
